...
Light::Light(std::pair<std::ofstream, uint32_t>&& lcd_backlight,
             ...
             std::ofstream&& red_duty_pcts, std::ofstream&& green_duty_pcts,
             std::ofstream&& red_start_idx, std::ofstream&& green_start_idx,
...
void Light::handleSpeakerBatteryLightLocked() {
    if (isLit(mNotificationState)) {
        ...
        setSpeakerLightLocked(mButtonsState);
    } else {
        // Lights off
        mRedLed << 0 << std::endl;
...
void Light::setSpeakerLightLocked(const LightState& state) {
    ...
    if (blink) {
        scaledDutyPcts = getScaledDutyPcts(brightness);
        ...
        mRedDutyPcts << scaledDutyPcts << std::endl;
        ...
        mGreenDutyPcts << scaledDutyPcts << std::endl;
